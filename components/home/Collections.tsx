...
const accentStyles: Record<Collection["accent"], { badge: string; numeral: string }> = {
  ...
  sand: { badge: "bg-forest", numeral: "text-moss" },
};
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {collections.map((collection, i) => {
            ...
            return (
              <article
                key={collection.id}
                className="group overflow-hidden rounded-card bg-white shadow-card transition-shadow duration-300 hover:shadow-lift"
              >
                <div className="relative">
                  ...
                  <span
                    className={cn(
                      "absolute -bottom-4 left-5 flex h-9 w-9 items-center justify-center rounded-full font-display text-sm font-extrabold text-ivory shadow-soft",
                      accent.badge,
                    )}
                  >
                ...
                <div className="px-6 pb-6 pt-7">
                  ...
                  <Link
                    href="#"
                    className="mt-5 inline-flex items-center gap-1.5 font-sans text-sm font-extrabold text-forest transition-colors duration-200 hover:text-burgundy"
                  >
                    {collection.cta}
                    <ArrowUpRight
                      size={17}
